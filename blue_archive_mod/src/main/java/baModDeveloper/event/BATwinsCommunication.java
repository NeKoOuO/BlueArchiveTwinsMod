...
public class BATwinsCommunication extends PhasedEvent {
    ...
    public BATwinsCommunication() {
        ...
        registerPhase("leave", new TextPhase(DESCRIPTIONS[1]).addOption(OPTIONS[2], integer -> {
            openMap();
        }));
        registerPhase("afterReward", new TextPhase(DESCRIPTIONS[2]).addOption(OPTIONS[2], integer -> {
            openMap();
        }));
